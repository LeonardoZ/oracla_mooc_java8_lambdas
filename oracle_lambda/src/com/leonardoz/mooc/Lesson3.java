...
public class Lesson3 {
	...
	static int[][] computeLevenshtein(List<String> wordList, boolean parallel) {
		...
		// Imperative as fuck, but faster.
		/*
		 * 
		 * if(parallel){ IntStream.range(0, LIST_SIZE).parallel() .forEach( i ->
		 * IntStream.range(i, LIST_SIZE).forEach( j -> distances[i][j] =
		 * Levenshtein.lev(wordList.get(i), wordList.get(j)) ) ); }else{ // YOUR
		 * CODE HERE IntStream.range(0, LIST_SIZE) .forEach( i ->
		 * IntStream.range(i, LIST_SIZE).forEach( j -> distances[i][j] =
		 * Levenshtein.lev(wordList.get(i), wordList.get(j)) ) ); } Looks like a
		 * functional code, but... and it's slower than the
		 * imperative-functional frankstein.
		 */
		
		/*
		 * Slow than the others
		 */
		
		int[][] array = 
			(parallel ? wordList.parallelStream() : wordList.stream())
			.map(w -> wordList.stream().mapToInt(w2 -> Levenshtein.lev(w, w2)).toArray())
			.collect(Collectors.toList()).toArray(distances);
		
		
//		if (parallel)
//			distances = IntStream.range(0, LIST_SIZE).parallel().mapToObj(
//					i -> wordList.parallelStream().mapToInt(w2 -> Levenshtein.lev(wordList.get(i), w2)).toArray())
//					.toArray(int[][]::new);
//		else
//			distances = IntStream.range(0, LIST_SIZE)
//					.mapToObj(i -> wordList.stream().mapToInt(w2 -> Levenshtein.lev(wordList.get(i), w2)).toArray())
//					.toArray(int[][]::new);
	...
	static List<String> processWords(List<String> wordList, boolean parallel) {
		// YOUR CODE HERE
		if (parallel)
			wordList = wordList.parallelStream().sorted().filter(w -> w.length() % 2 == 0).collect(Collectors.toList());
		else
			wordList = wordList.stream().sorted().filter(w -> w.length() % 2 == 0).collect(Collectors.toList());
